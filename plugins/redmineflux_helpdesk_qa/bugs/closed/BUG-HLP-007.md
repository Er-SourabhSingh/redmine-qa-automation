...
- Production issue #119631 found marked "In QA" (developer checked in a fix), triggering this retest per the user's request to retest all checked-in Helpdesk bugs on `localhost:3012`.
- Logged in as `luna.blossom` (Agent, Helpdesk QA Alpha) and navigated directly to the branded route `/projects/helpdesk-qa-alpha/helpdesk/issues/46` (still the only realistic way to reach it, per this bug's own established finding that there is no in-app agent click path to it) and separately `/projects/helpdesk-qa-alpha/helpdesk/issues/75`.
- **Result: both branded-route requests now redirect server-side to the core route** (`/issues/46` and `/issues/75` respectively) — confirmed by the final page URL after `goto`, not just a static `href` read. This matches the fix direction explicitly requested in this bug's own "Expected result" section (point 2): *"even if the branded route remains reachable, it should not be relied upon for Agent workflows."* An Agent can no longer land on `RfProjectHelpdeskIssuesController#update` at all — every branded-URL visit lands them on `IssuesController` instead.
- Completed the full original repro on the now-redirected core route (`/issues/46`): Reply → Internal Note radio → note text → Activity "Technical Support" → Save. **Result: "Successful update"**, no 500, no error — the note (`#2`) was saved and correctly marked **Private** in the ticket history.
- This confirms the underlying crash can no longer be triggered by any real Agent workflow: the buggy controller/action combination is no longer reachable by an Agent, and the equivalent action on the core route (already known-working) completes cleanly.
- Production issue #119631 synced 2026-09-10: status In QA → Done, % done → 100 (approved).
