...
| TC-TCM-523 (the bug's own stated retest vehicle) | *"email arrives as `multipart/mixed` carrying a valid `.pdf`"* — says nothing about body text |

**Conclusion: the user's reading is correct.** Bullet 1 and TC-TCM-523 both pass after the install. TC-TCM-524 —
...
- **TC-TCM-523 flipped to CONFIRMED PASS**; **TC-TCM-524** rewritten with its real preconditions (working
  Puppeteer + shell access to restart Sidekiq), a five-step procedure, and an explicit warning not to confuse it
  with TC-TCM-523. TC-TCM-525 note clarified as pre-install.
...
**Regression (partial, per §26 — High severity requires the affected suite + adjacent):** TC-TCM-521 (HTML email,
all six report types), TC-TCM-523 (PDF email) and TC-TCM-525 (report type does not affect emailing) all PASS in
the 2026-09-14 session; the HTML path was specifically confirmed not regressed by the installation. **The full
Reports-suite regression (TC-TCM-501…534) is still outstanding** — see Next Session Start Point.
...
- Authored **9 new test suites / 196 new test cases** covering all 18 features (CSV Import's 16 already existed):
  ...
  |---|---|---|
  | `TESTCASE_MANAGEMENT_ENVIRONMENTS.md` | TC-TCM-101–108 | 8 |
  | `TESTCASE_MANAGEMENT_TEST_SUITES.md` | TC-TCM-201–212 | 12 |
  | `TESTCASE_MANAGEMENT_TEST_CASES.md` | TC-TCM-301–324 | 24 |
  | `TESTCASE_MANAGEMENT_TEST_RUNS.md` | TC-TCM-401–440 | 40 |
  | `TESTCASE_MANAGEMENT_REPORTS.md` | TC-TCM-501–534 | 34 |
  | `TESTCASE_MANAGEMENT_REQUIREMENTS_RTM.md` | TC-TCM-601–616 | 16 |
  | `TESTCASE_MANAGEMENT_TODO.md` | TC-TCM-701–710 | 10 |
  | `TESTCASE_MANAGEMENT_PERMISSIONS.md` | TC-TCM-801–832 | 32 |
  | `TESTCASE_MANAGEMENT_CONFIGURATION.md` | TC-TCM-901–920 | 20 |
...
- Captured as **TC-TCM-017** in `testcases/TESTCASE_MANAGEMENT_CSV_IMPORT.md` (suite now 17 TCs), with fixture `automation/testdata/csv-test-data/18_all_columns_export_118789.csv`, screenshot `screenshots/RETEST-118789/`, log `logs/RETEST-118789-2026-09-15.log`.
...
- **Repeated on Redmine 6 (2026-09-15):** same test run end-to-end on `localhost:3012` (**Redmine 6.1.3**, plugin v7.0.0) after creating 15 custom fields there — 44 columns / 727-char header. **Step 4 rendered, no 500.** This closes the "does the fix hold on Redmine 6?" question, since the original defect was reported on Redmine 6.1.1. Both instances now covered by TC-TCM-017.
...
- **Full Reports-suite regression (TC-TCM-501…534) is outstanding.** BUG-TCM-005 was High severity, so §26 calls
  for the whole affected suite plus adjacent features; only TC-TCM-521, 523 and 525 have been re-run. This is the
  one gate item left over from closing that bug.
- **One untested case on BUG-TCM-006: the HTML format under a forced failure** (TC-TCM-524 step 6). Source
...
- **BUG-TCM-006 cannot be retested from the browser.** It needs shell access to break PDF generation (restart
  ...
  show a working PDF and produce a **false pass** — this is exactly the trap TC-TCM-524 now warns about.
...
- **Start executing the new suites.** None of the 196 new TCs has been executed. Suggested order by risk:
  ...
  2. `TESTCASE_MANAGEMENT_CONFIGURATION.md` TC-TCM-917–920 — establishes which apparent defects are really incomplete installs, before anything else is misfiled.
...
- Several TCs deliberately ask the tester to *record which of two behaviours occurs* rather than asserting one (e.g. TC-TCM-208, 311, 320, 902). Fill those in as they are executed — they are documentation gaps in the KB, not vague test cases.
...
- **BUG-TCM-005 (High)** — Report emailed as PDF arrives with no attachment. Retest PASS 2026-09-14 (TC-TCM-523):
...
| 2026-09-15 | 6.1.3 | Docker `localhost:3012` (project `test`) — desk review of 2026-09-14 evidence | Claude (no new live execution) | **BUG-TCM-005 rescoped and closed.** Original scope verified against the customer report, the bug title, both Expected-result bullets and TC-TCM-523 before changing anything. Test 1 (PDF delivered, 53,446-byte valid attachment) satisfies every one — root cause was an incomplete installation, not a code defect. Test 2's residual finding (a *failed* PDF still sends an email promising an attachment) split out as **BUG-TCM-006 (Medium)**; TC-TCM-524 moved to it as its retest vehicle and rewritten with real preconditions. TC-TCM-523 flipped to CONFIRMED PASS. `_duplicates.md` given a decision rule separating the two. Partial Reports regression: TC-TCM-521/523/525 PASS; **full suite regression outstanding.** 2 production writes queued pending approval. |
