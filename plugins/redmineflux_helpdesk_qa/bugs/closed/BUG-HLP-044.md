...
Per `HELPDESK_USER_GUIDE.md` §23 ("`rake redmineflux_helpdesk:auto_close_tickets` — Run the auto-close pass once") and `HELPDESK_RAKE_TASKS.md` TC-HLP-250 ("The eligible ticket is closed immediately — identical outcome to the scheduled auto-close job"), the rake task should produce the same result as the real scheduled `Helpdesk::AutoCloseTicketsWorker` — closing any eligible ticket per the project's real, currently-configured Auto Close Ticket Days.
...
- This is also the natural retest for TC-HLP-250 in `HELPDESK_RAKE_TASKS.md` ("`auto_close_tickets` run by hand closes eligible tickets") — now genuinely PASS, matching the scheduled worker's behavior as originally promised.
...
- Found while executing `HELPDESK_EMAIL.md` TC-HLP-077/153 — the rake task was tried first (as the lower-risk way to trigger the job), found to do nothing, and that investigation led directly to this root cause before falling back to invoking the real worker class to actually test the scheduled-job behavior (TC-HLP-081/BUG-HLP-043).
- This is the natural home for TC-HLP-250 in `HELPDESK_RAKE_TASKS.md` ("`auto_close_tickets` run by hand closes eligible tickets") — that TC's own Expected Result ("identical outcome to the scheduled auto-close job") is now confirmed FAIL; see that suite for the formal TC write-up.
