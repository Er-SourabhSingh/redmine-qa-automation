...
export class HelpdeskSlaPage extends BasePage {
  ...
  /**
   * Call after openProjectList() — clicks "New SLA", which is correctly
   * project-scoped by construction.
   *
   * BUG FIX 2026-09-14: same empty-state duplicate-link pattern as
   * HelpdeskCustomerPage.openNew() — on an EMPTY SLA list, "New SLA" renders
   * twice (header button + empty-state block), a strict-mode violation
   * never hit before since every prior run was against an environment with
   * existing SLAs. `.first()` is the header button either way.
   */
  async openNew() {
    await this.page.getByRole('link', { name: 'New SLA' }).first().click();
  }
  ...
  /**
   * True if an SLA with this name already exists on this project — added
   * 2026-09-14 for provisioning's check-before-create (Customer Project
   * Access silently requires at least one SLA to exist per project, or
   * customer creation fails — see provision.setup.ts's baseline-data step).
   */
  async exists(projectIdentifier: string, slaName: string): Promise<boolean> {
    await this.openProjectList(projectIdentifier);
    return this.row(slaName).isVisible().catch(() => false);
  }
