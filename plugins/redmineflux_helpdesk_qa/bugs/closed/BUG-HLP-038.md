...
- Production issue #120077 found marked "In QA" (developer checked in a fix), triggering this retest per the user's request to retest all checked-in Helpdesk bugs on `localhost:3012`.
- Setup: Alpha Minimal Fields Test Org, project Helpdesk QA Alpha, Hard mode. Reduced budget to Approved 15.67h / Used 14.67h / Remaining exactly 1.00h (via Add/top up hours, Hours = -6).
- As `luna.blossom` (agent), opened ticket #46, Reply → Time spent = Custom, 120 minutes (2h), Activity = Technical Support, Save.
- **Result: the entry was refused.** The form re-rendered with a validation error: *"Time entries is invalid — This entry (2.00h) is more than the 1.00h of prepaid support hours Alpha Minimal Fields Test Org has left. Log a smaller amount, or top up the budget first."* No time was logged; Spent time and the Prepaid Support Hours line were unchanged.
- This confirms the fix evaluates `remaining - requested_hours < 0` (or equivalent) as the gate, exactly matching this bug's recommended fix — Hard mode now blocks an oversized single entry instead of accepting it uncapped.
- Budget restored to baseline (21.67h Approved / 7.00h Remaining) immediately after the retest via Add/top up hours, Hours = +6.
- Production issue #120077 synced 2026-09-10: status In QA → Done, % done → 100 (approved).
