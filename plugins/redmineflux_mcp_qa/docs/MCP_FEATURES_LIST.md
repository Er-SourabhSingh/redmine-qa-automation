...
|---|---------|-------------|---------------|
| 1 | MCP server URL | Per-account, from Integrations → MCP Server | TC-RFM-101 |
| 2 | Redmine API key retrieval | My Account → API access key → Show / Reset | TC-RFM-102, 103 |
| 3 | Claude Desktop connection | `mcp-remote` bridge via `npx`, Node.js 18+ | TC-RFM-104 – 107 |
| 4 | Claude.ai web connection | Integrations → Model Context Protocol → Add Integration | TC-RFM-108, 109 |
| 5 | Claude Code CLI connection | `claude mcp add` with `--transport http`, project or `--scope user` | TC-RFM-110 – 112 |
| 6 | Claude Code config file | `.claude/mcp.json` or `~/.claude.json` | TC-RFM-113 |
| 7 | Connection verification | "Show me my Redmine projects" returns data | TC-RFM-114 |
| 8 | Plugin detection at startup | Only detected plugins' tools are registered | TC-RFM-201 – 205 |
| 9 | `redmineflux_system_version` | Reports version and detected plugins | TC-RFM-206, 207 |
| 10 | Detection refresh lag | Documented delay after install/uninstall | TC-RFM-208, 209 |
| 11 | Core tool coverage | ~90 core tools always available | TC-RFM-210 |
| 12 | Full tool coverage | 500+ with all plugins installed | TC-RFM-211 |
| 13 | Read operations | Issues, projects, time entries, boards, tickets, workload | TC-RFM-301 – 308 |
| 14 | Write operations | Create/update issues, log time, move cards, sprints, approvals | TC-RFM-401 – 410 |
| 15 | Custom field support | `custom_fields` parameter | TC-RFM-411 |
| 16 | Response formatting | Raw JSON formatted into a readable answer | TC-RFM-309, 310 |
| 17 | User-friendly error messages | Named as a product feature | TC-RFM-412 – 415 |
| 18 | Transient error retry | Automatic retry, per the KB | TC-RFM-416 |
| 19 | Per-user authentication | Each caller's own key and permissions | TC-RFM-901 – 904 |
| 20 | Permission enforcement | Actions limited to the key owner's rights | TC-RFM-905 – 910 |
| 21 | API key handling | Plain-text in client config; rotation via Reset | TC-RFM-911 – 913 |
| 22 | Data privacy | No Redmine data stored by the MCP server | TC-RFM-914 |
...
- **The single most important claim to verify is FAQ Q6**: *"The AI can only perform actions permitted by the
  ...
  would be reachable by simply asking for it. TC-RFM-905 to 910 exist for this and should be run before anything
  else.
...
- **The API key sits in plain text in client config files** (`claude_desktop_config.json`, `.claude/mcp.json`,
  `~/.claude.json`). The KB warns not to commit it to version control — TC-RFM-911 and 912 treat that as a real,
...
- **The KB's own convenience tip is a hazard worth recording**: "for full access without per-permission setup,
  ...
  administrator key handed to an AI assistant can do anything on the instance. TC-RFM-910 documents what that
  configuration actually permits, so the risk is stated rather than implied.
- **Plugin detection is stateful and lagging** (TC-RFM-208, 209). The KB documents staleness in both directions —
