...
| 9 | Inline edit — Custom fields | Configurable — format, Required, tracker scope, project scope, and (for List format) single- vs multi-select display type all change behavior | **Every format now tested and PASS** (TC-INE-012/413/414, 2026-09-22) except Key/value list (not creatable via the stock UI): Date, Text, Long text, Boolean, Integer, Float, Link, List (single + multi-select), User, Version all confirmed on the issue detail page; a representative sample (Boolean, Integer, User) also confirmed on the issue list; a project-level Text field confirmed on the project list. Attachment format confirmed to have no inline-edit affordance at all (by design). Widget-per-format and save-trigger-per-widget fully mapped — see `INLINE_EDITOR_MEMORY.md`. Configuration-driven behaviors (Required enforcement, tracker/project scoping) covered by TC-INE-001/408/409/410/411, all PASS. TC-INE-002–403 (tracker/project scoping) still not independently executed. |
| 14 | Workflow field permissions (readonly/required per role+tracker+status) | Redmine core Workflow → Edit tab grid; a field's editability/requiredness depends on the issue's current status | **PARTIAL PASS** — TC-INE-006/007/010/011 (status-dependent live behavior, executed 2026-09-22): the plugin correctly suppresses the inline affordance for a workflow-Read-only field and correctly falls back to the full Edit form when a status change makes a field Required, all PASS. TC-INE-057/094 (the same restriction, framed as a permission check) confirm the UI leg the same way. **The negative-endpoint leg (does the server, not just the UI, refuse the write?) remains unproven** for all of these — outstanding, needs a safe alternative to a hand-rolled `fetch()`. |
...
| 13 | Inline edit — Project card/list view (Name) | Per the plugin's own description: "project table, and project card views" | TC-INE-021/008 — PASS. Under Default theme only the card view was found (Name inline-editable, no strings to check). Under Lotus theme, a "Kartenansicht"/"Listenansicht" toggle also appears (not present under Default) — the list view (`?display_type=list`) is the plugin's own "project table" view, fully translated (Name/Kennung/Beschreibung headers, Filter panel), Name inline-editable, no bugs. A single project's own Overview page and the Administration → Projects admin table both have zero inline-edit icons — out of this plugin's scope. |
