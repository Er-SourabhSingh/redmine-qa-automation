...
- No open bugs. Pick up the outstanding list under "In Progress" above, roughly in this order:
  1. `INLINE_EDITOR_PERMISSIONS.md` TC-INE-097 (non-member/private-project, fixtures ready — issue #1554), TC-INE-099
     (cross-project write, same fixtures), TC-INE-101 (permission revocation — needs 2 simultaneous sessions,
     consider a second browser context), TC-INE-103 (visibility-scoped role — `summer.rain`/"QA Own Visibility"
     ready, not yet exercised).
  2. `INLINE_EDITOR_ISSUE_LIST_EDITING.md` TC-INE-067 (Status column specifically) and the remaining negative cases
     TC-INE-078/080/081/082/083/084/085/086.
  3. Agree a safe method for the "leg 3" negative-endpoint checks (TC-INE-093/094/095/096/097/104 step 3, plus
     TC-INE-INE-006/056/058 in the other suites) — this is the single highest-value remaining item per the suite's
     own text (TC-INE-094).
