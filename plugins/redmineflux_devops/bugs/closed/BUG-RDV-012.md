...
- Result: **FIXED**
- `GET /projects/phoenix-platform/devops_commits.json?limit=3&page=1` → HTTP 200, exactly 3 records returned (limit parameter now respected). Response also includes `total_count` field (was missing before). Both fixes confirmed. Bug no longer reproduces.
