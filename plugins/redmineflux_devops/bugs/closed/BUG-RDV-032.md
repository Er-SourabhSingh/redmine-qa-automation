...
- Result: **FIXED**
- Navigated to `/projects/phoenix-platform/devops_environments`. Production environment (env_id=3, locked until 2026-06-01) Deploy button: `disabled=true`. All other environments (dev, staging, qa, uat) have Deploy buttons enabled. Lock state now correctly disables the Deploy button in the UI. Bug no longer reproduces.
