...
- **Context**: fresh ticket #4 created via `alpha.customer`'s own portal New issue form (Status: New), zero shared history with the original ticket #70 repro.
- **Steps, matching the original repro exactly**: `luna.blossom` opened ticket #4 via genuine click-through from the in-app Helpdesk Tickets list (core `/issues/4` route), clicked the top-level **Edit** link (not Reply), entered Notes text, left **Private notes** unchecked, left Status untouched, clicked Submit.
- **Result — core defect (false Status auto-transition) does NOT reproduce**: Status remained **"New"** after submission — confirmed three independent ways: (1) the ticket page itself still shows "Status: New"; (2) no "Property changes" tab exists on the ticket at all (meaning zero field changes occurred, not just Status); (3) Luna Blossom's own Activity feed shows this ticket's entry with no "(Waiting for Customer Response)" tag, while the same feed's entry for ticket #3 (where a genuine Reply Note was used earlier this session) correctly does show that tag — a clean side-by-side contrast confirming the plugin now correctly distinguishes a plain Edit-form note from an actual Reply.
- **Secondary observation, not part of this bug's core scope**: unlike the original repro (where the Edit-form note correctly stayed out of the customer's Notes tab), this retest's note **is now visible** in `alpha.customer`'s own Notes tab (`/projects/helpdesk-qa-alpha/helpdesk/issues/4?tab=notes`). This is a behavior difference from the original report, but it's the opposite direction of a problem — a non-private standard Redmine note being visible to someone with view access to the issue is normal core Redmine behavior, not a defect; the original bug was never about note visibility being wrong (its own Expected Result explicitly says Notes tab exclusion "is correct, not a defect" and the real problem was always the status transition). Flagging this only so a future session doesn't mistake it for something new — not filing a separate bug for it, and not counting it against this bug's own verdict.
- Screenshots: `retest-2026-08-31-status-unchanged-no-false-transition.png` (agent view, ticket #4), `retest-2026-08-31-customer-view-no-conversion-tab.png` (customer view, still no Helpdesk Conversion tab at all since no real reply channel fired).
- **Verdict: RETEST PASS.** The status-auto-transition defect this bug is actually about no longer reproduces — submitting a plain Edit-form note leaves Status untouched, exactly as expected.

## Closed — 2026-08-31

- Closed per explicit user confirmation, following the clean retest above (Status no longer falsely auto-transitions on a plain Edit-form note, confirmed three independent ways).
- If the false auto-transition reappears on any path, file a new bug rather than reopening this one.
