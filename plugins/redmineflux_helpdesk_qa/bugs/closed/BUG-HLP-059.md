...
- Bug ID: BUG-HLP-059
- Production Redmine Issue ID: #120543 (ztflux) — reopened on production again 2026-09-21 after retest (developer had moved it back to In QA; moved to Reopen again since the list-endpoint gap still reproduces)
...
## Retest — 2026-09-21 (Local, `redmine-docker-6`, production issue #120543 checked in again)

**PARTIALLY FIXED, one of the two remaining gaps closed, kept OPEN — the zero-ticket-visibility gap on the list endpoint still reproduces exactly as before.**

- **`restrict_helpdesk_customer_access` allowlist gap — CONFIRMED FIXED.** Source now has an explicit early return: `return if params[:controller].to_s.start_with?('api/v1/')` (`application_controller_patch.rb`, with a `BUG-HLP-059 (reopened)` comment explaining the original filter never gave the API's own auth layer a chance to run). Live-verified: logged in as `retest.customer1` via a real session (no API key at all), called `GET /helpdesk/api/v1/tickets` and `GET /helpdesk/api/v1/tickets/15` (her own ticket) — neither redirected away (`redirected: false`, real JSON both times); `tickets/15` returned her own ticket's real subject correctly. This is a genuine fix for the session-based half of the original finding.
- **New partial improvement found while retesting — individual ticket lookup (`show`) now works for key-only access too.** The original finding reported `GET /tickets/:id` returning a `404 "Ticket not found."` for a customer's own ticket via key-only (no session) access. Retested with a fresh key for `retest.customer1`: `GET /helpdesk/api/v1/tickets/15` (a real ticket she authored) now returns a genuine `200` with the correct ticket data. This specific regression is fixed.
- **Zero-ticket-visibility gap on the LIST endpoint (`GET /tickets`) — STILL BROKEN, reproduces identically to the original finding.** `GET /helpdesk/api/v1/tickets` returns `200 {"data":[],"meta":{"total_count":0,...}}` for a real customer with real authored tickets — confirmed on **two different customers** (`retest.customer1`, 29 real authored Support tickets; `alpha.customer`, 2 real authored tickets), via **both** auth methods (key-only header, and a real logged-in session with no key), with cache-busting query params and `credentials: 'omit'` ruling out caching/cookie artifacts. A same-request baseline with **admin's own key** on the identical endpoint correctly returns `200 {"total_count": 392}` (all 392 real Support tickets, unfiltered as expected for a non-customer) — proving the endpoint and its count mechanism work correctly in general; the defect is specifically in how it resolves the customer's own scope for `index`.
  - Source (`app/controllers/api/v1/tickets_controller.rb#build_ticket_scope`) reads as if this should already be fixed — `scope = scope.where(author_id: User.current.id) if User.current.is_helpdesk_customer?` — and manually replicating that exact query chain via `rails runner` with `User.current` set to the same customer returns the correct count (29). The discrepancy between that manual reconstruction and the live HTTP behavior was not root-caused to an exact line this session (ruled out: stale code — file's last modified date predates the container's last two boots; tracker-lookup ambiguity — only one `Support` tracker exists; HTTP caching — cache-busted and still empty; cookie interference — tested with `credentials: 'omit'`). Flagging the unresolved root cause for a future session or the original developer, since black-box behavior is conclusively, repeatedly confirmed broken regardless of mechanism.
- **Net effect, unchanged from the bug's original framing**: a customer still cannot practically use the documented REST API to see a list of their own tickets — she can now reach a ticket she already knows the ID of (`show`, newly fixed), but the one operation actually needed to discover what tickets she has (`index`/list) still returns nothing. Kept OPEN.
- Production issue #120543 synced 2026-09-21: developer had moved it back to In QA since the 2026-09-18 reopen; moved back to **Reopen** with the full retest evidence above (both new fixes confirmed, list-endpoint gap still reproduces).

## Retest — 2026-09-22 (Local, `redmine-docker-6`, production issue #120543 checked in again): list-endpoint gap CONFIRMED FIXED, all 4 original sub-issues now resolved

**FIXED — the remaining list-endpoint gap from the 2026-09-21 retest is now resolved. Moved to closed/.**

- `GET /helpdesk/api/v1/tickets` via key-only auth for `retest.customer1` now returns `200 {"meta":{"total_count":29}}` — exactly matching her real authored Support-tracker ticket count (previously `0`).
- Cross-checked with a second customer, `alpha.customer` (2 real authored tickets): correctly returns `total_count: 2`, with the two real subjects listed.
- Re-confirmed the admin-key baseline on the identical endpoint still correctly returns all 392 tickets unfiltered (not a regression in the other direction).
- Re-confirmed session-based access (logged in as `retest.customer1`, no API key at all): `GET /helpdesk/api/v1/tickets` returns the same correct `total_count: 29`, not redirected away, and `GET /helpdesk/api/v1/tickets/15` (show) still returns her own ticket correctly.
- **All four originally-cited/discovered sub-issues across this bug's full retest history are now confirmed fixed**: (1) session-cookie auth works, (2) customers can self-serve an API key via `/my/api_key`, (3) the `restrict_helpdesk_customer_access` filter no longer blocks `api/v1/*` for a logged-in customer, (4) the ticket list correctly scopes to the customer's own authored tickets. No root cause was ever pinned down for why the list endpoint returned 0 the day before — it may have been a stale-process artifact from that session's own migration/restart sequence rather than a genuine unfixed code path, since the source already read as correct at that time too.
- Production issue #120543 synced 2026-09-22: Status In QA → **Done**, % Done → **100**.

## Notes

- Found while executing `HELPDESK_REPORTING_AUTOMATION.md` TC-HLP-279 (REST API customer data isolation) — that TC's own precondition anticipates customers having *some* API access to test isolation against; in practice there is none to test, for the reasons above.
