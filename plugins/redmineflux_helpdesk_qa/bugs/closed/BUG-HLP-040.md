...
Per `HELPDESK_PERMISSIONS.md` TC-HLP-206: a role granted `manage_prepaid_support_hours` (with `view_helpdesk`, without `manage_helpdesk`) should be able to top up the budget and change the run-out mode — "Both actions succeed even though this role lacks `manage_helpdesk`."
