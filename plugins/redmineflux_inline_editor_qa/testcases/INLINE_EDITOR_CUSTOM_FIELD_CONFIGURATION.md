...
> Source: gap analysis against `INLINE_EDITOR_ISSUE_DETAIL_EDITING.md` TC-INE-306 (custom fields tested as one
> blanket "every type" case) and `INLINE_EDITOR_PERMISSIONS.md` TC-INE-320/903 (workflow read-only fields tested
...
> TC-INE-407, TC-INE-409, TC-INE-410, TC-INE-411, TC-INE-412, TC-INE-413 and TC-INE-414 all PASS; TC-INE-406
> PARTIAL PASS (endpoint leg outstanding); TC-INE-401/408 confirmed via TC-INE-409. Every Redmine custom field
> format now exercised except Key/value list (not creatable via the stock UI), across all 4 inline-editable
> surfaces (issue detail, issue list, project list, project board). Still to execute: TC-INE-402, 403, and
> TC-INE-406 step 3.**
>
...
> |---|---|---|---|
> | Custom-field-level (`is_required`) | A non-status field (Priority) | TC-INE-409 | PASS |
> | Workflow-level (per status) | **Status** | TC-INE-407 | PASS |
> | Custom-field-level (`is_required`) | **Status** | TC-INE-410 | PASS |
> | **Both at once** (Required + Read-only-at-New, same status) | Issue **creation**, then Priority, then Status | TC-INE-411 | PASS — no deadlock; Redmine exempts a read-only field from Required enforcement entirely |
...
|---|---|---|---|---|
| QA Required Text Field | `cf_63` | Text | Required ✓, all trackers, all projects | TC-INE-401, TC-INE-408, TC-INE-409 |
| QA Second Required Field | `cf_64` | Text | Required ✓, all trackers, all projects | TC-INE-408, TC-INE-409 |
| QA Bug-Only Tracker Field | `cf_65` | Text | Bug tracker only, all projects; **Required ✓ added 2026-09-22** for TC-INE-410 | TC-INE-402, TC-INE-410 |
| QA Test-Project-Only Field | `cf_66` | Text | All trackers, "test project" only (not "For all projects") | TC-INE-403 |
| QA Multi Select Field | `cf_67` | List | Multiple selection ✓, values Red/Green/Blue/Yellow, all trackers/projects | TC-INE-404 |
| QA Single Select Field | `cf_68` | List | Multiple selection ✗, same values, all trackers/projects | TC-INE-405 |
| QA Workflow Field | `cf_69` | Text | Bug tracker, all projects; Workflow → Fields permissions (role **Developer**, tracker **Bug**): status **New** = **Read-only**, **In Progress** = **Required**, Resolved = blank | TC-INE-406, TC-INE-407 |
| QA Required Readonly Field | `cf_70` | Text | **Required ✓ set at creation**, Bug tracker, all projects; Workflow (Developer/Bug): **New = Read-only**, every other status = blank/unselected | TC-INE-411 |
| QA Boolean Field | `cf_71` | Boolean | All trackers, all projects | TC-INE-412 |
| QA Integer Field | `cf_72` | Integer | All trackers, all projects | TC-INE-412 |
| QA Float Field | `cf_73` | Float | All trackers, all projects | TC-INE-412 |
| QA Long Text Field | `cf_74` | Long text | All trackers, all projects | TC-INE-412 |
| QA Link Field | `cf_75` | Link | All trackers, all projects | TC-INE-412 |
| QA User Field | `cf_76` | User | All trackers, all projects | TC-INE-412 |
| QA Version Field | `cf_77` | Version | All trackers, all projects | TC-INE-412 |
| QA Attachment Field | `cf_78` | Attachment | All trackers, all projects | TC-INE-412 (confirmed out of inline-edit scope) |
| QA Project Text Field | `cf_79` (**project-level**) | Text | Default (no scoping) | TC-INE-414 |
...
through `cf_65`/`cf_69` history) and **#1552** (created fresh by `daisy.skye` specifically for TC-INE-411, so it
...
### TC-INE-401: Custom field "Required" checkbox is enforced via inline editor
...
- **Already confirmed as part of TC-INE-409 below** (which exercises this exact fixture): the inline save of the
  unrelated field is rejected server-side and the plugin falls back to rendering the full standard Edit form with
  the attempted change preserved — see TC-INE-409 for the full evidence. This TC remains to independently confirm
  step 2 (inline-editing `cf_63` itself while it's blank) hits the same enforcement.

---

### TC-INE-402: Custom field scoped to a specific tracker only appears inline on issues of that tracker
...
### TC-INE-403: Custom field scoped to specific project(s) — not "For all projects"
...
**Expected Result:**
- Absent on the excluded project's issues, both UI and endpoint. Same failure mode as TC-INE-402 (silent write
...
### TC-INE-404: List-format custom field with "Multiple selection" enabled
...
### TC-INE-405: List-format custom field WITHOUT "Multiple selection" (single-value)

**User Role:** Member
**Preconditions:** "QA Single Select Field" (`cf_68`) is List format, Multiple selection ✗, same Red/Green/Blue/
Yellow values as `cf_67` — the direct contrast fixture for TC-INE-404.
**Steps:**
...
**Expected Result:**
- Single-value semantics preserved — this is the baseline case TC-INE-404 is contrasted against, confirming the
...
### TC-INE-406: A read-only-by-status field shows no inline affordance, and becomes editable once the status changes
...
2. Move the issue to "In Progress" (via TC-INE-407's path), then re-check `cf_69`'s affordance.
...
- At "New", `cf_69` renders with **no inline edit affordance at all**, and the endpoint refuses a write to it.
  ...
  the write is TC-INE-903's core risk and would be High severity.
...
- **Step 2 PASS:** once the issue reached "In Progress" (see TC-INE-407), `cf_69`'s inline pencil **appeared** on
...
### TC-INE-407: A field becomes "Required" at a specific status via workflow, enforced through inline editor

**User Role:** Developer (`daisy.skye`)
**Preconditions:** Same `cf_69` rule as TC-INE-406 — Status "New" = Read-only, "In Progress" = **Required**. Issue
...
**Steps:**
1. On issue #1551 at Status "New", confirm `cf_69` is blank and not inline-editable (TC-INE-406 step 1).
...
3. Instead of a dead-end error, the plugin **opened the full standard Edit form in place** (URL stayed
   `/issues/1551`, no hard navigation) — the same graceful fallback confirmed in TC-INE-409. In that form:
...
### TC-INE-408: Multiple custom fields required simultaneously
...
**Result: PASS, confirmed as part of TC-INE-409 below** — both fields' "cannot be blank" errors were reported
together in a single list, not one-at-a-time. See TC-INE-409 for the full evidence; this TC and TC-INE-401 both
resolve to the same underlying behavior TC-INE-409 exercises directly.

---

### TC-INE-409: Inline-editing another field while required fields are unmet falls back to the full Edit form
...
### TC-INE-410: Custom-field-level Required + status change — no error at rest, Edit form on the transition
...
> TC-INE-409 = non-status field edit + **custom-field-level** Required; TC-INE-407 = **Status** change +
...
### TC-INE-411: A field that is BOTH Required (custom-field-level) AND Read-only-at-New — no deadlock
...
### TC-INE-412: Every remaining custom field format, inline-edited on the issue detail page

> Added per explicit user instruction: "also test custom field each types." Combined with the Date (TC-INE-328)
> and List single/multi-select (TC-INE-404/405) coverage already executed, this closes out every Redmine custom
...
### TC-INE-413: List format's two widgets, executed (multi-select save mechanism + single-select contrast)

Extends TC-INE-404/405 (previously authored but not executed) with the actual execution evidence.
...
- **Single-select (`cf_68`, plain native `<select>`):** selecting "Green" saved immediately on the native
  ...
  pattern. This is the correct baseline contrast for TC-INE-404/405: the plugin does read the field's own
...
### TC-INE-414: Regression sweep — custom field formats across all 4 inline-editable surfaces
...
- **`cf_71` (Boolean) — genuine surface difference found, not a bug:** on the **issue list**, Boolean renders as a
  **plain native `<select>`** (`None`/`Yes`/`No`), saving instantly on `change` — a completely different widget
  from the **`rf-ss` searchable dropdown** it uses on the **issue detail page** (see TC-INE-412). Both work
...
**Issue Detail** — see TC-INE-412/413 above (the primary site of this session's format testing); no separate
...
|---------|------------|-----|---------------|
| TC-INE-414 | — | Issue list: Integer 42→99 (200), User→Luna Blossom id 111 (200), Boolean→No via native select (200). Project list: `cf_79` text saved (204). Project board: Name round-tripped, both saves 204 | — |
| TC-INE-411 | — | `cf_70` absent from create form → issue #1552 created clean, `cf_70` blank → Priority inline-save succeeds directly at "New" (no fallback needed) → Status→In Progress rejected (3 fields incl. `cf_70`) → Edit form in place → filled all 3 → submitted → Status=In Progress, all persisted | — |
| TC-INE-412 | — | 8 formats all confirmed via reload: Boolean=Yes, Integer=42, Float=3.14, Long Text=set (Ctrl+Enter), Link=set (renders `<a>`), User=Daisy Skye (id 112), Version=sadfsadfsad, Attachment=no affordance (by design) | — |
| TC-INE-413 | — | Multi-select: outside-click discards (confirmed blank on reload) → explicit Save persists `["Red","Blue"]` → remove+add → Save → `["Blue","Yellow"]` (2 identical 200 calls, duplicate-save non-finding). Single-select: native `change` saves "Green" immediately, no Save button | — |
| TC-INE-410 | — | At "New": zero error elements, `cf_65` blank+editable, `cf_69` no pencil. Status→In Progress: "Qa bug-only tracker field cannot be blank" → Edit form in place, status preserved, `cf_65` editable → filled+submitted → Status=In Progress, value saved | — |
| TC-INE-406 | — | At "New" as Developer: `cf_69` row has zero `.rf-edit-icon`/button (read-only honored in UI). At "In Progress": pencil present. Endpoint leg outstanding. | — |
| TC-INE-407 | — | Inline Status New→In Progress with `cf_69` blank → rejected "Qa workflow field cannot be blank" → full Edit form opened in place with Status preserved and `cf_69` editable → filled + submitted → Status=In Progress, `cf_69` set | — |
| TC-INE-409 | — (no bug; behavior confirmed via DOM/UI state inspection, not screenshot — no bugs found, screenshots are bug-evidence only per §6) | Attempted inline PATCH → server-side validation reject → full Edit form rendered in place, attempted value preserved, resubmission succeeded | — |
