...
- User role: `luna.blossom` (Manager) with "Edit issues"/"Edit own issues" temporarily unchecked on the Manager role (found while executing TC-CRX-158, `CRUX_PROJECT_CREATION_AND_IMPROVE_WAND.md`)
- Date: 2026-09-15
...
- Confirmed the same underlying refusal (no write occurs) is correctly enforced from a pure permission-boundary standpoint — see TC-CRX-160, which passed on that narrower question. This bug is specifically about the dishonest **reporting** of that refusal, not about the refusal itself failing to happen.
...
- After restoring "Edit issues"/"Edit own issues" on the Manager role and retrying the identical Apply flow (TC-CRX-156), the write succeeded cleanly with exactly one journal entry ("Description updated") — confirming the earlier attempt was a real, cleanly-recoverable refusal, not a partial/corrupted write.
