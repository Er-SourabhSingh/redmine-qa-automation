...
- Same session, same user (`manage.helpdesk.test`): SLA "TC-HLP-167 Permission Test SLA" created successfully (redirect to SLA list, "Successful creation."); Support Level "TC-HLP-167 Permission Test Level" created successfully; Organization "TC-HLP-167 Permission Test Org" created successfully; Product "TC-HLP-167 Permission Test Product" created successfully; Holiday "TC-HLP-167 Permission Test Holiday" created successfully.
...
- TC-HLP-167 in `HELPDESK_PREPAID_HOURS.md`'s sibling suite already established Support Packages (also global/install-wide) is `manage_helpdesk`-gated correctly, not admin-only — worth cross-referencing when this bug is triaged, since it undercuts an "all global entities are admin-only by design" defense.
