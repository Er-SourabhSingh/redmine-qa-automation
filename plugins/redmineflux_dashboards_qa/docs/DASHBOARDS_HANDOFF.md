...
- TC-DSH-001: dashboard shell (heading, header controls, global filter bar, empty state) — FAIL. Almost every plugin-owned string is hardcoded English despite a confirmed-German session. Filed `BUG-DSH-001` (High).
- TC-DSH-002: "Add New Chart" modal — FAIL, same root cause. All 22 chart types listed and functional; spot-checked 5 by name, all untranslated. "Abbrechen" is the one correctly-translated string (shared core component).
- TC-DSH-003: chart card controls (added "Issues by Status") — FAIL, same root cause. Icon row and resize-handle labels all untranslated.
- TC-DSH-004: Chart Settings panel (General/Data Filters/Appearance, all 3 sections) — FAIL, same root cause.
