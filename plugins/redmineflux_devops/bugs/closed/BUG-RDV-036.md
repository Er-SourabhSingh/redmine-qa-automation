...
- Result: **FIXED**
- Created rollback of deployment #5 via `POST /projects/phoenix-platform/devops_deployments/5/create_rollback` → redirected to new rollback deployment #16. Returned to deployment #5 detail page: "Reverted" indicator is now present. The original deployment now shows the reverted-by reference. Bug no longer reproduces.
