...
- Production issue #120086 found marked "In QA" (developer checked in a fix), triggering this retest per the user's request to retest all checked-in Helpdesk bugs on `localhost:3012`.
- Setup: Alpha Minimal Fields Test Org, Hard mode, reduced to exactly 1.00h remaining (Approved 15.67h / Used 14.67h).
- **Path A (edit)**: as `luna.blossom`, opened `/time_entries/21/edit` (an existing entry at 0:10), changed Hours to `3:00` (a 2.83h increase against 1.00h remaining), Save. **Result: blocked** — *"This entry (2.83h) is more than the 1.00h of prepaid support hours Alpha Minimal Fields Test Org has left. Log a smaller amount, or top up the budget first."* No change was saved; the entry remains at its original 0:10.
- **Path B (core "Log time" new-entry link)**: as `luna.blossom`, opened `/issues/46/time_entries/new` (the native Redmine new-entry form), entered Hours `5:00` (far exceeding the 1.00h remaining), Create. **Result: blocked** — identical validation error: *"This entry (5.00h) is more than the 1.00h of prepaid support hours Alpha Minimal Fields Test Org has left."* No entry was created.
- Both previously-uncapped core `TimeEntry` code paths (edit and new-entry) now enforce Hard mode identically to the reply-box widget — confirming the fix followed this bug's own recommendation to route enforcement through a shared, model-level concern rather than a single controller action.
- Budget restored to baseline (21.67h Approved / 7.00h Remaining) immediately after the retest.
- Production issue #120086 synced 2026-09-10: status In QA → Done, % done → 100 (approved).
