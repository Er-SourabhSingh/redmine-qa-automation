...
## Retest — 2026-09-22: RECONFIRMED for new-ticket-creation specifically, time-log path stays solid

**The original symptom (successful over-budget action under Hard mode) was observed again, per the trigger condition set in the correction above. Verdict upgraded from Unconfirmed to a real, reproducible defect — but now precisely scoped: it affects new-ticket creation specifically, not both paths.**

- **Setup**: Organization "Alpha Minimal Fields Test Org" (id 8), Helpdesk QA Alpha project, Hard mode confirmed selected. Reduced the budget via a single **-6.75h** operation (matching the original repro's "one direct reduction to exact zero" shape) so Approved = 14.92h = Used = 14.92h, **Remaining = 0.00h exactly** — confirmed on the org page, the customer's own project dashboard ("100% Used"), and the ticket detail panel, all simultaneously.
- **Part 1 — new ticket, as `delta.customer`**: clicked New issue on Helpdesk QA Alpha, filled Subject + the required "test" custom field, clicked Create. **Result: NOT blocked** — flash message "Successful creation.", a real, persisted ticket (**#407**, "BUG-HLP-037 retest 2026-09-22 - Hard mode at zero hours new ticket test") appeared in the tickets list. Confirmed via the ticket list itself (9 tickets, was 8) — this is the exact same failure mode as the original 2026-09-03 finding, not a fluke of this single click.
- **Part 2 — time log on an existing ticket, as `luna.blossom`**: opened ticket #46 (already in this org/project, "0.00h left" shown on its own detail panel), clicked Reply, entered a note with Time spent = 10 min, Activity = Technical Support, clicked Save. **Result: correctly blocked** — a real validation error: *"Time entries is invalid — This entry (0.17h) is more than the 0.00h of prepaid support hours Alpha Minimal Fields Test Org has left. Log a smaller amount, or top up the budget first."* No time was logged, no journal entry created (cancelled the form afterward).
- **Combined picture across all retests to date** (original + this session's 2026-09-03 correction's 3 retests + this retest):
  - **New-ticket-creation path**: FAILED to block on the original finding, FAILED to block on this retest (2 failures) — blocked correctly only once, in the correction's own Retest 3. **Majority of independent attempts show this path is not reliably enforced.**
  - **Time-log path**: FAILED to block only on the original finding, blocked correctly on all of Retest 1/2/3 in the correction *and* this retest (4 successes, 1 failure). **This path is reliably enforced.**
