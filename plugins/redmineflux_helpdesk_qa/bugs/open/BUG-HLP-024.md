...
- **2026-09-01, original filing**: scoped to Path A only (an admin manually reassigning an *already-assigned* higher-level ticket to a lower-level agent via the standard Edit form), following a user correction from an initial "not a bug" verdict on TC-HLP-338.
...
- `HELPDESK_SLA_ESCALATION.md` TC-HLP-338 (Path A) updated the same session with the corrected expectation; TC-HLP-342/343/344 (Paths B/C and the no-false-positive control) added fresh to cover the broadened scope, all with live evidence.
