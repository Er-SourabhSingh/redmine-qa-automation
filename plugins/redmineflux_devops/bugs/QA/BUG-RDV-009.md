...
- Result: **CONFIRMED** (path-specific)
- `GET /devops/metrics/dora.json?project_id=phoenix-platform` → HTTP 404. The documented endpoint path still does not exist on local.
- Separate route `GET /projects/phoenix-platform/devops_metrics.json` → HTTP 200 with DORA data. The data is accessible via a different route format, but the documented `/devops/metrics/dora.json` path remains unrouted. Bug remains open.
...
- Result: **CONFIRMED**
- `GET /devops/metrics/dora.json?project_id=agileboard` (admin API key) → HTTP 404. Documented DORA endpoint still absent on new Forge. Bug persists.
...
- Result: **CONFIRMED**
- `GET /devops/metrics/dora.json?project_id=agileboard` (admin API key) → HTTP 404. Documented DORA endpoint absent on new Forge instance. Bug persists.
