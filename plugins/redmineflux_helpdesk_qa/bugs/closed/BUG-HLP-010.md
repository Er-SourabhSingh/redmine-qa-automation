...
- Bug ID: BUG-HLP-010
- Production Redmine Issue ID: 119656
...
- **Steps, matching the original repro exactly**: created "Alpha Org" with Website `https://acme.example.com` and Phone `+1 (555) 123-4567`, then attempted to create a second, differently-named organization ("Alpha Org Subsidiary") with the exact same Website and Phone values.
- **Result**: form submission was **refused outright** — "New Organization" page reloaded with two validation errors: **"Phone number has already been taken"** and **"Website has already been taken"**. The record was not created; both fields' values remained in the form for correction. Screenshot: `retest-2026-08-31-duplicate-website-phone-refused.png`.
- **Verdict: RETEST PASS.** This is a genuine fix, not merely a soft warning — it achieves the strongest tier the original "Expected result" asked for ("ideally it should require confirmation or refuse outright"). Website and Phone Number are now unique across organizations, matching Name's existing uniqueness behavior.
- Deleted the throwaway "Alpha Org" afterward via the standard confirmation-modal delete flow (the rejected "Alpha Org Subsidiary" attempt was never actually saved, so nothing to clean up there).
- **Note for whoever updates the requirements/test docs**: `HELPDESK_USER_GUIDE.md` §3.4 and `testcases/HELPDESK_FIELD_VALIDATIONS.md` TC-HLP-096 both currently describe the old (permissive) behavior as correct/expected — both need updating now that Website/Phone are enforced unique, per this bug's own original Notes section anticipating exactly this follow-up.

## Closed — 2026-08-31

- Closed per explicit user confirmation, following the clean retest above (duplicate Website/Phone are now refused outright, stronger than the minimum the bug asked for).
- `HELPDESK_USER_GUIDE.md` §3.4 and TC-HLP-096 still need their own updates to reflect this — not done as part of this closure, tracked separately.

## Notes

- Raised during a QA coverage review: the user asked whether Organization Website/Contact uniqueness was tested, and after confirming live that duplicates are currently allowed (see `testcases/HELPDESK_FIELD_VALIDATIONS.md` TC-HLP-096, which documents this as PASS against the *current* spec), the user pushed back that this should arguably be unique regardless of what the doc says. Filed as Low severity per that explicit direction.
- **Triage note for whoever picks this up**: this is a product-judgment bug, not a spec-deviation bug — the fix (if pursued) is a new validation rule that doesn't exist in the requirements today, not a repair of broken existing logic. Confirm intended behavior with product/stakeholders before implementing; TC-HLP-096 will need its own "Expected Result" flipped (PASS → the new refusal/warning behavior) if this is fixed, since right now it correctly documents today's actual (permissive) behavior.
