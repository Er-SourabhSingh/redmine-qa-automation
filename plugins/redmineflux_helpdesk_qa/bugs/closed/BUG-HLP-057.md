...
- Found while executing `HELPDESK_REPORTING_AUTOMATION.md` TC-HLP-277 ("Export refused for a role without `export_helpdesk_reports`") — that TC's own precondition assumes a role can view Reports but be refused Export specifically; no such tier exists, since viewing Reports at all already requires `manage_helpdesk`, which already includes export capability by itself.
- Also directly relevant to TC-HLP-261, whose own "Agent with `view_helpdesk`" precondition needed correcting for the same underlying reason (`view_helpdesk` alone cannot reach Reports either — see that TC's evidence).
