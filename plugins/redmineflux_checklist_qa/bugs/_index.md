...
| BUG-CHK-005 | Checklist CRUD (create, toggle, delete) is NOT blocked on a closed/read-only project — only "Add from template" is, and even that gives no user-facing feedback | Open — retested 2026-09-24: writes now blocked (403), but silent-failure/no-feedback part still unresolved on all 4 actions | High → Low (remaining scope) | 7.0.0 (Docker) | bugs/open/BUG-CHK-005.md |
