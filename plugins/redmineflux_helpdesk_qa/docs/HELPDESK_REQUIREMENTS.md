...
3. **SLA breach → escalation**: a missed deadline marks the ticket breached, notifies, and the background SLA monitor (every 2 min) escalates the ticket to the next support level, reassigning and notifying the new tier. A breach at the top level notifies admins but cannot escalate further.
...
| Manage desk configuration (SLAs, support levels, organizations, products, canned responses, holidays) | Yes | Yes, if granted `manage_helpdesk` | No | No |
| Export reports | Yes | Only if granted `export_helpdesk_reports` | No | No |
| Set/adjust prepaid budgets | Yes | Only if granted `manage_prepaid_support_hours` | No | No |
