...
- Session 2026-09-21: fetched production issue #120919 (Feature, status In QA) — a fix for the inline date editor
  ...
  TC-INE-323–328 (`INLINE_EDITOR_ISSUE_DETAIL_EDITING.md`) and TC-INE-225–226 (`INLINE_EDITOR_ISSUE_LIST_EDITING.md`)
  ...
  test session.
- Session 2026-09-22: executed TC-INE-323–328 and TC-INE-225–226 live against local Docker `redmine-docker-700`
...
- Session 2026-09-22 (continued): user asked whether a large list of custom-field-configuration and permission
  ...
  `INLINE_EDITOR_CUSTOM_FIELD_CONFIGURATION.md` (TC-INE-401–408: Required enforcement, tracker-scoped fields,
  ...
  TCs in `INLINE_EDITOR_PERMISSIONS.md` (TC-INE-913–915: "Edit own issues" vs "Edit issues" distinction, "Edit
...
- Session 2026-09-22 (continued): user asked to first explore the instance's actual custom fields before writing
  ...
  In Progress=Required, Resolved=Read-only). Rewrote TC-INE-401–408's preconditions to reference these real
  fixtures by name/ID instead of hypothetical placeholders. **Authored and executed live** a new TC-INE-409
...
- Session 2026-09-22 (continued): user asked to explore Workflow → Fields permissions and prepare a fixture for
  ...
  (admin is exempt from workflow field permissions). **TC-INE-406 PARTIAL PASS, TC-INE-407 PASS**: read-only field
  shows no affordance at New; transitioning to In Progress while blank is rejected and opens the Edit form fallback
  with the status preserved and the field now editable. Endpoint-level enforcement of the Read-only rule (TC-INE-406
...
- Session 2026-09-22 (continued): user asked the sharpest version — a field Required from creation AND
  ...
  **New TC-INE-411, PASS, no deadlock**: `cf_70` is fully absent (not disabled) from the New-issue form; issue
...
- Session 2026-09-22 (continued): user asked to run a regression pass across issue detail/list, project list, and
  ...
  (`cf_79`). **TC-INE-412/413/414 all PASS** — every format works on the issue detail page (mapped each format's
