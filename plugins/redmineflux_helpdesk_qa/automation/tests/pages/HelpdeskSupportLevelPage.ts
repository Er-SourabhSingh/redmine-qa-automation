...
export class HelpdeskSupportLevelPage extends BasePage {
  ...
  /**
   * Call after openProjectList() — clicks "New Support Level", which is
   * correctly project-scoped by construction.
   *
   * `.first()` pre-emptively (2026-09-14): the same empty-state
   * duplicate-link pattern was confirmed live on SLA's own "New SLA" link
   * and Customer's "New Customer" link (header button + empty-state block,
   * both rendering the same link on an empty list) — same shared UI
   * component, so the same risk almost certainly applies here too even
   * though no run has hit an empty Support Level list yet.
   */
  async openNew() {
    await this.page.getByRole('link', { name: 'New Support Level' }).first().click();
  }
  ...
  /**
   * True if a Support Level with this name already exists on this project —
   * added 2026-09-14 for provisioning's check-before-create (Customer
   * Project Access silently requires at least one Support Level to exist
   * per project, or customer creation fails — see provision.setup.ts's
   * baseline-data step).
   */
  async exists(projectIdentifier: string, supportLevelName: string): Promise<boolean> {
    await this.openProjectList(projectIdentifier);
    return this.row(supportLevelName).isVisible().catch(() => false);
  }
   ...
   *
   * BUG FIX 2026-09-14: a bare `getByText(name, {exact:true})` matches BOTH
   * the visible widget's `<label>` AND a hidden native `<option>` backing it
   * (confirmed live via a real strict-mode violation — this codepath had
   * never actually run before, since every prior Support Level fixture was
   * created manually). Scoped to `label` specifically, with an anchored
   * exact-text regex (not `hasText`'s default substring match) to also
   * avoid colliding with a longer name that happens to contain this one.
   */
  async selectAssignees(names: string[]) {
    await this.assigneesField.click();
    for (const name of names) {
      const exact = new RegExp(`^${name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`);
      await this.page.locator('label').filter({ hasText: exact }).click();
    }
  }
