...
## Retest — 2026-09-21 (Local, `redmine-docker-6`, production issue #120465 checked in)

**FIXED — confirmed via source and a genuine, clean end-to-end live nesting test. Moved to closed/. Production issue #120465 synced to Status: Done, % Done: 100 (2026-09-21).**

- **Source-level fix**: a real migration adds the missing `parent_id` column (`db/migrate/20260918140000_add_parent_id_to_rf_knowledgebase_pages.rb`); the model (`app/models/rf_knowledgebase_page.rb`) now declares `belongs_to :parent` / `has_many :children` plus two real validations (a parent must be in the same project; a page can't be its own ancestor); the controller's `page_params` now permits `parent_id`; and the JS (`assets/javascripts/rf_knowledgebase.js`) now sends `parent_id` in `createNewPage()`'s payload when a page is open, and `buildTree()` now actually nests the flat list the server returns instead of rendering everything as root-level — all changes carry explicit `BUG-HLP-048` comments.
- **Live confirmation**: with "Resetting your password" (page 3) open, pressed `r` to open "New Article", saved a new child ("BUG-HLP-048 retest v2 - clean post-restart nesting test") — it was created with `parent_id: 3`, the breadcrumb read "Resetting your password / BUG-HLP-048 retest v2 - clean post-restart nesting test", and the sidebar tree correctly rendered it nested under the parent with an expand arrow, not as a root-level sibling.
- **Operational note (shared with BUG-HLP-049's retest)**: the first two create attempts this session hit a `500` because the new migration above hadn't been applied yet — `rake db:migrate` runs automatically on this container's restart, but `rake redmine:plugins:migrate` does not unless `REDMINE_PLUGINS_MIGRATE` is set. Once the migration was applied and the container/Sidekiq restarted cleanly, nesting worked with zero errors on a fresh attempt. This is an environment/deploy-procedure gotcha, not a remaining defect in the fix itself — see BUG-HLP-049's retest notes for the same finding, and consider adding this to root `MEMORY.md` for any future plugin-migration deploy on this container.
- **Follow-up coverage per user request — the sidebar tree's own controls beyond the "r"/global-"+" path also confirmed working:**
  - **Multi-level nesting (grandchild)**: opened the just-created child page (id 13) itself, pressed `r` again — the "New Article" modal correctly used the now-open *child* as the parent, not the original root article. Saved a new page (id 16) which nested 3 levels deep: breadcrumb read "Resetting your password / BUG-HLP-048 retest - real nesting test child / BUG-HLP-048 retest - grandchild multi-level nesting test", and the sidebar tree rendered it correctly nested under the child, which itself nests under the root. Confirms nesting isn't limited to one level.
  - **Per-row Delete icon on a sub-page**: each child/grandchild row in the tree gets its own `<span class="kb-icon ... delete-node" id="delete-<id>">` (confirmed via DOM — root-level pages get no action icons at all, only child/grandchild rows do). Clicking it on the grandchild (id 16) opened a real "Delete Page?" confirm modal ("Are you sure you want to delete this page? This action cannot be undone."); confirming removed it cleanly from the tree and the page genuinely no longer exists (`RfKnowledgebasePage.where(id: 16).exists?` → `false`).
  - **Clarified for the record**: there is no separate per-row "+" (add-child) icon anywhere in the tree — confirmed via source (`createAllNodes`/`loopChild` in `rf_knowledgebase.js`): root rows render no action span at all, and child rows' action span renders only the Delete icon, never an add icon. The single `#add-space` "+" next to the "Knowledgebase" heading (tooltip "Add Article") is the only add control in the UI, and it calls the same `openChildModal()`/`createNode()` path as the `r` shortcut, using whichever page is currently open as the parent — already covered by the tests above.
