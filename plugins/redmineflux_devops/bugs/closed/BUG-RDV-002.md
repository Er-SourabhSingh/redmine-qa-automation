...
- Result: **FIXED**
- 2,200,156-byte payload (>2 MB, valid HMAC-SHA256) → HTTP 413 `{"error":"payload too large","code":"payload_too_large"}`. Size enforcement is now working. Bug no longer reproduces.
...
- Result: **FIXED**
- 2.2 MB JSON payload → HTTP 413. Size limit enforced on new Forge. Bug does not reproduce.
