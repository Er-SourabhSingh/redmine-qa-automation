...
As a companion positive control (not itself a bug): TC-HLP-077 confirmed a genuinely Resolved ticket (#55) DOES correctly close under the identical run — the mechanism works, it's just far broader in scope than documented or than the UI's own field hint promises.
...
- Sidekiq log line at the moment of closure: `[Auto-Close] ✓ Closed issue #56: TC-HLP-081 auto-close test ticket B (never Resolved, stays In Progress)`.
...
- Duplicate found: No (checked `bugs/_duplicates.md` — empty register; grepped the whole plugin QA folder for "auto close"/"auto-close" beforehand — this exact scenario, testing a non-Resolved ticket against the real worker, was written as TC-HLP-081 but never previously executed)
