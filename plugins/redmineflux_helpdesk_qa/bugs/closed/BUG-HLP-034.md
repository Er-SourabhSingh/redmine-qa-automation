...
- Production issue #120025 found marked "In QA" (developer checked in a fix), triggering this retest per the user's request to retest all checked-in Helpdesk bugs on `localhost:3012`.
- Created two independent fresh tickets (#76 "retest ticket A", #77 "retest ticket B") as `retest.customer1` (entitlement: Alpha Escalation Test SLA, L2), assigned both to `autumn.grace` (an L2 agent) as admin to start each ticket's own SLA clock at L2.
- Waited for both L2 response deadlines to pass, then triggered `Helpdesk::SlaMonitorWorker.new.perform` once, processing both breaches in the same cycle.
- **Result**: `Ticket #76 | L2 → L3 | Assignee: autumn.grace → aurora.wren` and `Ticket #77 | L2 → L3 | Assignee: autumn.grace → willow.belle` — **two different agents**, not the same lowest-ID agent every time as originally reported (previously both tickets landed on `willow.belle`).
- This confirms escalation into a multi-agent Support Level now distributes across its configured assignees rather than deterministically picking `.first` on an unordered query.
- Production issue #120025 synced 2026-09-10: status In QA → Done, % done → 100 (approved).
