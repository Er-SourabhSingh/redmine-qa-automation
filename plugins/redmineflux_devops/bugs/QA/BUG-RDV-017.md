...
- Result: **CONFIRMED**
- `GET /projects/phoenix-platform/devops_commits` with no authentication → HTTP 200 (not redirected to login, not 403). Unauthenticated access to the commits page returns full content. Dev_user (view_devops) can also access PR list (HTTP 200 with dev_user API key). Permission guard not enforced. Bug persists.
...
- Result: **CONFIRMED**
- Temporarily removed `view_devops` from Developer role. daisy.skye (no DevOps permissions): `GET /projects/agileboard/devops_pull_requests` → HTTP 200; `GET /projects/agileboard/devops_commits` → HTTP 200. Both pages returned full content without `view_devops`. Restored `view_devops` after test. Bug persists.
