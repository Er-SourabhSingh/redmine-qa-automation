...
- Result: **FIXED**
- Navigated to `/projects/phoenix-platform/devops_repositories/new`. Provider dropdown now includes "Argocd" (value="argocd") as an option alongside Github, Gitlab, Bitbucket, Fossa. ArgoCD is now in SUPPORTED_PROVIDERS. POSTing to `/devops/webhook/argocd/phoenix-platform` returns HTTP 403 (signature_invalid) not 400 (provider not found), confirming provider lookup succeeds. Bug no longer reproduces.
